...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="py-3 mb-4"><span class="text-muted fw-light">Beranda /</span> Cetak Bukti Keikutsertaan</h4>
    <div class="card">
        <h5 class="card-header">Cari Kartu Keikutsertaan Acara</h5>
...
<script>
    $(document).ready(function() {
        ...
        $('#acara, #email').on('change', function() {
            let acara_id = $('#acara').val();
            let email = $('#email').val();

            if(acara_id && email) {
                $.ajax({
                    url: "{{ route('cek.kartu.peserta') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        acara_id: acara_id,
        ...
        $('#unduh').on('click', function() {
            ...
            window.location.href = "{{ url('peserta/kartu-peserta/unduh') }}/" + uuid;
        });
    });
</script>
@endpush
